import numpy as np
import pyopencl as cl
from matplotlib import pyplot as plt
import sys

# Read beta value from command-line arguments
if len(sys.argv) > 1:
    beta_0 = float(sys.argv[1])
else:
    beta_0 = 1.0  # Default value

print(f"Using β = {beta_0:.2f}")

##############################################################################
# 1) OPENCL SETUP
##############################################################################
platform = cl.get_platforms()[0]
device = platform.get_devices()[0]
context = cl.Context([device])
queue = cl.CommandQueue(context)

with open('ser_lindblad.cl', 'r') as f:
    kernel_code = f.read()
program = cl.Program(context, kernel_code).build()

##############################################################################
# 2) SIMULATION PARAMETERS
##############################################################################
dt = .001           # Smaller time-step => fewer numerical leaps
total_time = 100.0
num_steps = int(total_time / dt)
time_axis = np.linspace(0, total_time, num_steps + 1)


##############################################################################
# 3) SYSTEM SIZE SELECTION
##############################################################################
DSIZE = int(input("Enter matrix size (2 for 2x2, 4 for 4x4): ").strip())

##############################################################################
# 4) SYSTEM MATRICES (Dynamic DSIZE)
##############################################################################
# rho_init = np.eye(DSIZE, dtype=np.complex128) / DSIZE  # Normalized identity
A = np.random.randn(DSIZE, DSIZE) + 1j * np.random.randn(DSIZE, DSIZE)
rho_init = A @ A.conj().T  # Make it positive semidefinite
rho_init /= np.trace(rho_init)  # Normalize to trace = 1

# Hamiltonian and Lindblad operators - simple example
H_init = np.random.rand(DSIZE, DSIZE) + 1j * np.random.rand(DSIZE, DSIZE)
H_init = (H_init + H_init.conj().T) / 2  # Hermitian

L_init = np.random.rand(DSIZE, DSIZE) + 1j * np.random.rand(DSIZE, DSIZE)
L_init = (L_init + L_init.conj().T) / 2  # Hermitian

I_init = np.eye(DSIZE, dtype=np.complex128)

##############################################################################
# 5) CREATE OPENCL BUFFERS
##############################################################################
mf = cl.mem_flags
rho_buf = cl.Buffer(context, mf.READ_WRITE | mf.COPY_HOST_PTR, hostbuf=rho_init.flatten())
H_buf = cl.Buffer(context, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=H_init.flatten())
L_buf = cl.Buffer(context, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=L_init.flatten())
I_buf = cl.Buffer(context, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=I_init.flatten())

##############################################################################
# 6) ARRAYS TO STORE PURITY, ENTROPY, COHERENCE, TRACE
##############################################################################
purity_vals = np.zeros(num_steps + 1)
entropy_vals = np.zeros(num_steps + 1)
coherence_vals = np.zeros(num_steps + 1)
trace_vals = np.zeros(num_steps + 1)

##############################################################################
# 7) HELPER FUNCTIONS
##############################################################################
def compute_quantities(rho_1d):
    """Compute purity, entropy, coherence, trace from a DSIZE x DSIZE density matrix."""
    rho_m = rho_1d.reshape(DSIZE, DSIZE)
    rho2 = rho_m @ rho_m
    purity = np.real(np.trace(rho2))

    w = np.linalg.eigvalsh(rho_m)
    w_pos = w[w > 1e-12]  # Ignore tiny negative drift
    entropy = -np.sum(w_pos * np.log2(w_pos)) if len(w_pos) > 0 else 0.0

    coherence = np.sum(np.abs(rho_m)) - np.trace(np.abs(rho_m))
    trace = np.real(np.trace(rho_m))

    return purity, entropy, coherence, trace

def enforce_positivity(rho_m):
    """Project rho_m onto the positive semidefinite cone."""
    w, v = np.linalg.eigh(rho_m)
    w_clamped = np.clip(w, 0, None)
    rho_corrected = v @ np.diag(w_clamped) @ v.conj().T
    return rho_corrected / np.trace(rho_corrected)


def compute_entropy_derivative(rho_prev, rho_curr, dt):
    """Compute entropy change rate dS/dt."""
    w_prev = np.linalg.eigvalsh(rho_prev)
    w_curr = np.linalg.eigvalsh(rho_curr)

    S_prev = -np.sum(w_prev[w_prev > 1e-12] * np.log2(w_prev[w_prev > 1e-12]))
    S_curr = -np.sum(w_curr[w_curr > 1e-12] * np.log2(w_curr[w_curr > 1e-12]))

    return (S_curr - S_prev) / dt


def compute_energy_derivative(rho_prev, rho_curr, H, dt):
    """Compute energy change rate dE/dt."""
    E_prev = np.real(np.trace(rho_prev @ H))
    E_curr = np.real(np.trace(rho_curr @ H))

    return (E_curr - E_prev) / dt


##############################################################################
# 8) INITIALIZATION (step = 0)
##############################################################################
rho_host = np.empty_like(rho_init)
cl.enqueue_copy(queue, rho_host, rho_buf).wait()
rho_m = rho_host.reshape(DSIZE, DSIZE)
rho_m = enforce_positivity(rho_m)
rho_host = rho_m.flatten()

p0, e0, c0, t0 = compute_quantities(rho_host)
purity_vals[0] = p0
entropy_vals[0] = e0
coherence_vals[0] = np.real(c0)
trace_vals[0] = t0

cl.enqueue_copy(queue, rho_buf, rho_host).wait()

print(f"Step 0, Purity={p0:.4f}, Entropy={e0:.4f}, Coherence={c0:.4f}, Trace={t0:.4f}")

##############################################################################
# 9) MAIN LOOP
##############################################################################
hbar = 1.0
entropy_derivative_vals = np.zeros(num_steps)
energy_derivative_vals = np.zeros(num_steps)

for step in range(1, num_steps + 1):
    # Store previous state
    rho_prev = rho_host.reshape(DSIZE, DSIZE).copy()

    coherence_prev = coherence_vals[step - 1]
    gamma = 0.3 * np.exp(-2.0 * (1.0 - coherence_prev))

    norm_entropy = entropy_vals[step - 1] / np.log2(DSIZE) if step > 1 else 0.0
    # beta_0 = 1.0  # Baseline beta value
    beta = beta_0 * (1 - np.exp(norm_entropy))
    beta = min(beta, 2.0)

    # Kernel Execution with dynamic DSIZE
    program.ser_lindblad(
        queue, (DSIZE,), None,
        rho_buf, H_buf, L_buf, I_buf,
        np.float64(hbar), np.float64(gamma), np.float64(beta), np.float64(dt)
    )

    cl.enqueue_copy(queue, rho_host, rho_buf).wait()
    rho_m = rho_host.reshape(DSIZE, DSIZE)
    rho_m = enforce_positivity(rho_m)
    rho_host = rho_m.flatten()

    p, e, c, t = compute_quantities(rho_host)
    purity_vals[step] = p
    entropy_vals[step] = e
    coherence_vals[step] = np.real(c)
    trace_vals[step] = t

    # Compute entropy derivative
    dS_dt = compute_entropy_derivative(rho_prev, rho_m, dt)
    entropy_derivative_vals[step - 1] = dS_dt

    # Compute energy derivative
    dE_dt = compute_energy_derivative(rho_prev, rho_m, H_init, dt)
    energy_derivative_vals[step - 1] = dE_dt


    if step % 10000 == 0:
        print(f"Step {step}, Purity={p:.4f}, Entropy={e:.4f}, "
              f"Coherence={c:.4f}, Trace={t:.4f}, gamma={gamma:.4f}, beta={beta:.6f}")

##############################################################################
# 10) FINAL OUTPUT AND PLOTTING
##############################################################################
print("-- Final Values ---")
print(f"Purity = {purity_vals[-1]:.6f}")
print(f"Entropy = {entropy_vals[-1]:.6f}")
print(f"Coherence = {coherence_vals[-1]:.6f}")
print(f"Trace = {trace_vals[-1]:.6f}")

cumulative_energy = np.cumsum(energy_derivative_vals) * dt

# Save simulation data
np.save("rho_final.npy", rho_host)
np.savetxt("rho_final.csv", rho_host.reshape(DSIZE, DSIZE).view(float), delimiter=",")
np.savetxt("SER_entropy_derivative.csv", np.column_stack((time_axis[:-1], entropy_derivative_vals)),
           delimiter=",", header="Time,dS_dt", comments="")
np.savetxt("SER_energy_derivative.csv", np.column_stack((time_axis[:-1], energy_derivative_vals)),
           delimiter=",", header="Time,dE_dt", comments="")

# Save evolution results
results = np.column_stack((time_axis, purity_vals, entropy_vals, coherence_vals))
np.savetxt("SER_results.csv", results, delimiter=",", header="Time,Purity,Entropy,Coherence", comments="")

# Generate and save plots
plt.figure(figsize=(10, 6))
plt.plot(time_axis, purity_vals, label='Purity')
plt.plot(time_axis, entropy_vals, label='Entropy')
plt.plot(time_axis, coherence_vals, label='Coherence')
plt.xlabel('Time')
plt.ylabel('Quantity')
plt.legend()
plt.title('Evolution of Purity, Entropy, Coherence')
plt.grid(True)
plt.savefig("SER_plot.png", dpi=300)
plt.savefig("SER_plot.pdf")
plt.close()

time_axis = np.linspace(0, total_time, num_steps + 1)
plt.figure(figsize=(10, 6))
plt.plot(time_axis, purity_vals, label='Purity')
plt.plot(time_axis, entropy_vals, label='Entropy')
plt.plot(time_axis, coherence_vals, label='Coherence')
plt.plot(time_axis, trace_vals, label='Trace', linestyle='--')
plt.xlabel('Time')
plt.ylabel('Quantity')
plt.legend()
plt.title('Evolution of Purity, Entropy, Coherence, and Trace')
plt.grid(True)
plt.show()

plt.figure(figsize=(10, 6))
plt.plot(time_axis[:-1], entropy_derivative_vals, label='dS/dt', color='red')
plt.xlabel('Time')
plt.ylabel('Entropy Rate (dS/dt)')
plt.title('Entropy Evolution under SER Feedback')
plt.grid(True)
plt.legend()
plt.savefig("SER_entropy_rate.png", dpi=300)
plt.show()

plt.figure(figsize=(10, 6))
plt.plot(time_axis[:-1], energy_derivative_vals, label='dE/dt', color='blue')
plt.xlabel('Time')
plt.ylabel('Energy Injection Rate (dE/dt)')
plt.title('Energy Evolution under SER Feedback')
plt.grid(True)
plt.legend()
plt.savefig("SER_energy_rate.png", dpi=300)
plt.show()

trace_var = np.max(trace_vals) - np.min(trace_vals)
print(f"Trace variation: {trace_var:.1e}")

# Plot cumulative energy over time:
plt.figure(figsize=(10,6))
plt.plot(time_axis[:-1], cumulative_energy, label='Cumulative Energy Injected by SER')
plt.xlabel('Time')
plt.ylabel('Cumulative Energy')
plt.title('Cumulative Energy Flow due to SER Feedback')
plt.grid(True)
plt.legend()
plt.savefig("SER_cumulative_energy.png", dpi=300)
plt.show()
