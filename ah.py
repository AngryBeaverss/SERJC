...
feedback_strengths = [0.1, 0.3, 0.6]  # Different β_max values

# Coupling strengths to test
coupling_strengths = [3.25 / GHz_to_MHz, 20 / GHz_to_MHz, 50 / GHz_to_MHz]  # Weak, strong, ultra-strong
...
for g in coupling_strengths:
    for beta_max in feedback_strengths:
        ...
        # Define Lindblad Master Equation with Feedback
        def lindblad_rhs(t, rho_flat, H_base, L_list, beta_max):
            """ ODE function for the density matrix evolution with feedback. """
...
plt.plot(sol.t, concurrence_over_time, label=f'g = {g * GHz_to_MHz:.2f} MHz, β = {beta_max:.2f}')
plt.xlabel('Time')
...
plt.figure(figsize=(10, 6))
plt.plot(sol.t, ergotropy_over_time, label=f'g = {g * GHz_to_MHz:.2f} MHz, β = {beta_max:.2f}')
plt.xlabel('Time')
...
plt.figure(figsize=(10, 6))
plt.plot(sol.t, cavity_photon_occupation, label=f'g = {g * GHz_to_MHz:.2f} MHz, β = {beta_max:.2f}')
plt.xlabel('Time')
...
plt.figure(figsize=(10, 6))
plt.plot(sol.t, qubit_population, label='Qubit Excitation Probability')
plt.plot(sol.t, ergotropy_over_time, label='Extractable Work (Ergotropy)', linestyle='dashed')
plt.xlabel('Time')
